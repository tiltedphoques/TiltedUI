...
OverlayRenderProcessHandler::OverlayRenderProcessHandler(std::string aCoreObjectName /* = "core" */)
    : m_coreObjectName(aCoreObjectName)
{
}
...
CefRefPtr<CefV8Value> OverlayRenderProcessHandler::ConvertValue(CefRefPtr<CefValue> apValue)
{
    ...
    case VTYPE_LIST:
    {
        ...
        auto pCovertedList = CefV8Value::CreateArray(static_cast<int>(pOriginalList->GetSize()));

        for (size_t i = 0; i < pOriginalList->GetSize(); ++i)
        {
            pCovertedList->SetValue(static_cast<int>(i), ConvertValue(pOriginalList->GetValue(static_cast<int>(i))));
        }

        return pCovertedList;
    }
    case VTYPE_DICTIONARY:
    {
        auto pOriginalDict = apValue->GetDictionary();
        auto pCovertedDict = CefV8Value::CreateObject(nullptr, nullptr);
        ...
        {
            pCovertedDict->SetValue(keys[i], ConvertValue(pOriginalDict->GetValue(keys[i])), V8_PROPERTY_ATTRIBUTE_NONE);
        }

        return pCovertedDict;
    }
    }
...
void OverlayRenderProcessHandler::CreateEvent(const std::string& acEventName)
{
    m_pCoreObject->SetValue(acEventName, CefV8Value::CreateFunction(acEventName, m_pEventsHandler), V8_PROPERTY_ATTRIBUTE_NONE);
}

void OverlayRenderProcessHandler::CreateFunction(const std::string& acFunctionName)
{
